...
exports.createPage = (req, res) => {
    ...
    Pages.findOne({ category: req.body.category }).exec((error, page) => {
        if (error) return res.status(400).json({ errors:error });
        if (page) {
            Pages.findOneAndUpdate({ category: req.body.category }, req.body).exec(
                (error, updatedPage) => {
                    if (error) return res.status(400).json({ errors:error });
                    if (updatedPage) {
...
exports.getPage = (req, res) => {
    ...
    if (type === "page") {
        Pages.findOne({ category: category }).exec((error, page) => {
            if (error) return res.status(400).json({ errors:error });
